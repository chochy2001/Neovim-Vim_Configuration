...
return {
    -- nvim-lspconfig provides server definitions in lsp/ directory
    {
        ...
        config = function()
            ...
            local function configure_servers()
                        ...
                        settings = {
                            Lua = {
                                ...
                                workspace = {
                                    ...
                                    ignoreDir = { ".git", "tmp", "node_modules", ".dart_tool", "build" },
                                },
                ...
                -- client would duplicate the Flutter integration.
                ...
                if vim.fn.executable(astro_mason) == 1 or vim.fn.executable("astro-ls") == 1 then
                    vim.lsp.config("astro", {
                        capabilities = capabilities,
                        cmd = vim.fn.executable(astro_mason) == 1 and { astro_mason, "--stdio" }
                            or { "astro-ls", "--stdio" },
                        filetypes = { "astro" },
                ...
                if vim.fn.executable("bash-language-server") == 1 then
                    vim.lsp.config("bashls", {
                        ...
                        settings = {
                            ...
                            bashIde = { shellcheckPath = vim.fn.exepath("shellcheck") },
                        },
                    })
            ...
            configure_servers()
            -- Mason adds its bin directory before the executable guards run.
            -- Repeat after first-time installs so the new server uses our settings.
            require("mason-registry"):on("package:install:success", vim.schedule_wrap(configure_servers))
            ...
            vim.api.nvim_create_autocmd("LspAttach", {
                ...
                callback = function(ev)
                    ...
                    vim.keymap.set("n", "<leader>fm", function()
                        require("formatting").format(ev.buf)
    ...
    },
    {
        ...
        opts = {
            ensure_installed = vim.env.NVIM_OFFLINE == "1" and {} or {
                "lua_ls",
                "clangd",
                "jsonls",
                "yamlls",
            ...
            automatic_enable = false, -- only configure_servers owns enablement
        },
    },
    ...
    -- companion to mason-lspconfig, which only covers LSP servers)
    {
        ...
        opts = {
            ensure_installed = vim.env.NVIM_OFFLINE == "1" and {} or {
                ...
                -- Provision them with `uv tool install black` and
                -- `uv tool install clang-format`
                -- instead; none-ls picks them up from PATH.
            },
            auto_update = false,
            run_on_start = vim.env.NVIM_OFFLINE ~= "1",
            start_delay = 3000,
            debounce_hours = 24,
        },
    },
